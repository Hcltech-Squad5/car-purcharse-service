...
class CarImageServiceTest {
    ...
    private Car createCar(Integer id) {
        Car car = new Car();
        car.setId(id);
        // You might want to set other basic fields if 'Car' object is used beyond just its ID
        return car;
    }

    // Corrected Helper method to create a CarImage object
    // Now accepts a 'Car' object instead of an 'Integer carId'
    ...
    @BeforeEach
    void setUp() {
        // No specific setup needed here for @Mock and @InjectMocks
    }
    ...
    @Test
    @DisplayName("delete should throw RuntimeException if car image not found for deletion")
    void delete_carImageNotFound_throwsRuntimeException() {
        Integer nonExistentImageId = 99;

        when(carImageRepository.findById(nonExistentImageId)).thenReturn(Optional.empty());

        RuntimeException thrown = assertThrows(RuntimeException.class, () -> {
            carImageDaoService.delete(nonExistentImageId);
        });

        assertEquals("The Car Image is not found", thrown.getMessage());
